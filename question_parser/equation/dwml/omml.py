...
, LIM_FUNC, LIM_TO, LIM_UPP, M, BRK, BLANK, BACKSLASH, ALN, FUNC_PLACE, TEXT_CHARS)
...
def escape_latex(strs):
    if strs == '' or strs is None:
        return D_DEFAULT['null']
    last = None
    new_chr = []
    strs = strs.replace(r'\\', '\\')
    strs = strs.replace('（', '(')
    ...
    for c in strs:
        ...
        elif (c in TEXT_CHARS) and (last != BACKSLASH):
            new_chr.append(r'\text{%s}' % c)
        else:
            new_chr.append(c)
        last = c
...
def get_val(key, default='', store=CHR):
    if key:
...
class oMath2Latex(Tag2Method):
    ...
    def do_spre(self, elm):
        """
        the Pre-Sub-Superscript object
        """
        children = self.process_children_dict(elm)
        return '{}{}{}'.format(children['sub'], children['sup'], children['e'])
    ...
    def do_nary(self, elm):
        ...
        for stag, t, e in self.process_children_list(elm):
            if stag == 'naryPr':
                bo = get_val(t.chr if t.chr else u'\u222b', store=CHR_BO)
            else:
                res.append(t)
    ...
    def do_r(self, elm):
        ...
        """
        _str = []
        ts = elm.findtext('./{0}t'.format(OMML_NS))
        if ts is None:
            return ""
        for s in ts:
